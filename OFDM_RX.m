function [M_n,Threshold_graph,H_hat_time,RX_Payload_1_no_Equalizer,RX_Payload_2_no_Equalizer,RX_Payload_1_no_pilot,RX_Payload_2_no_pilot,BER] = OFDM_RX(RX,Parameters_struct)
    ...
    %% Packet Detection (based on Schimdl & Cox algorithm)

    D = 16;     % Delay (Short preamble length)
    L = 2*D;    % Autocorrelation window length

    C_k = zeros(1,L); % Buffer arrays
    P_k = zeros(1,L);

    C_n = zeros(1,length(RX_signal)-D+1-L); % Final arrays
    P_n = zeros(1,length(RX_signal)-D+1-L);
    
    
    for n=1:length(RX_signal)-D+1-L
        for k=1:L
            C_k(k) = RX_signal(n+k-1)*complex(RX_signal(n+k+D-1));
            P_k(k) = abs(RX_signal(n+k+D-1))^2;
        end
        C_n(n) = sum(C_k);
        P_n(n) = sum(P_k);
    end
    M_n = abs(C_n)./P_n; % Got rid of the square
    ...
    Packet_Front = (temp_3(1:end-1) > 400);  % Find replaced by logical indexing
    ...
    OVR = 2;
    Frame_DWN_sampling = RX_signal(idx:OVR:OVR*624+idx-1); % [1x624]
    idx_2 = idx;
    Frame_DWN_sampling_2 = RX_signal_2(idx_2:OVR:OVR*624+idx_2-1); % [1x624]
